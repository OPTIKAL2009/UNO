...
class UnoCard:
    ...
    def _validate(self, color, card_type):
        ...
        if color not in ALL_COLORS:
            raise ValueError(_('Invalid color'))
        if color == 'black' and card_type not in BLACK_CARD_TYPES:
            raise ValueError(_('Invalid card type'))
...
class UnoPlayer:
    ...
    def __init__(self, cards, player_id=None):
        if len(cards) != 7:
            raise ValueError(
                _('Invalid player: must be initalised with 7 UnoCards')
            )
    ...
    def __repr__(self):
        if self.player_id is not None:
            return _('<UnoPlayer object: player {}>').format(self.player_id)
        else:
            return _('<UnoPlayer object>')
...
class UnoGame:
    ...
    def __init__(self, players, random=True):
        if not isinstance(players, int):
            raise ValueError(_('Invalid game: players must be integer'))
        if not 2 <= players <= 15:
            raise ValueError(_('Invalid game: must be between 2 and 15 players'))
    ...
    def play(self, player, card=None, new_color=None):
        ...
        if not isinstance(player, int):
            raise ValueError(_('Invalid player: should be the index number'))
        if not 0 <= player < len(self.players):
            raise ValueError(_('Invalid player: index out of range'))
        _player = self.players[player]
        if self.current_player != _player:
            raise ValueError(_('Invalid player: not their turn'))
        if card is None:
        ...
        if not self.current_card.playable(_card):
            raise ValueError(
                _('Invalid card: {} not playable on {}').format(
        ...
        if _card.color == 'black':
            if new_color not in COLORS:
                raise ValueError(
                    _('Invalid new_color: must be red, yellow, green or blue')
                )
    ...
    def _print_winner(self):
        ...
        print(_("Player {} wins!").format(winner_name))
...
class AIUnoGame:
    def __init__(self, players):
        ...
        print(_('The game begins. You are Player {}.').format(self.player_index))
        self.print_hand()
    ...
    def __next__(self):
        ...
        if player == self.player:
            print(_('Current card: {}, color: {}').format(
            ...
            if player.can_play(current_card):
                ...
                while not played:
                    ...
                    if not game.current_card.playable(card):
                        ...
                        if card.color == 'black':
                            new_color = input(_('Which color do you want? '))
                        else:
                            new_color = None
            ...
            for i, card in enumerate(player.hand):
                if game.current_card.playable(card):
                    if card.color == 'black':
                        ...
                        new_color = None
                    print(_("Player {} played {}").format(player, card))
